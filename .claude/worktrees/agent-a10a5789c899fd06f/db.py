...
class Database:
    ...
    def get_daily_cost_data(self, days: int = 30) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of daily cost records with date, total_wh, and cost_usd
        """
        cursor = self.conn.cursor()

        # Calculate the cutoff timestamp for the specified number of days ago
        cutoff_timestamp = int(time.time()) - (days * 24 * 3600)

        # Query to get daily aggregated cost data from combined_metrics
        # The cost_data JSON contains total_wh and cost_usd fields
        cursor.execute(
            """
            SELECT
                date(timestamp, 'unixepoch') as date,
                SUM(json_extract(cost_data, '$.total_wh')) as total_wh,
                SUM(json_extract(cost_data, '$.cost_usd')) as cost_usd,
        ...
        for row in cursor.fetchall():
            results.append({
                "date": row["date"],
                "date_timestamp": int(time.mktime(datetime.strptime(row["date"], "%Y-%m-%d").timetuple())),

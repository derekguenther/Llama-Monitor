...
<body>
    ...
    <script>
        ...
        function formatDate(dateStr) {
            // Input: "2026-06-11", Output: "06/11/2026"
        ...
        // Charts
        let gpuChart, cpuChart, powerChart, costChart;
...
def api_monthly_cost():
    """Return daily cost data for the last month."""
    config = get_config()
    db_path = getattr(config, "database_path", "llama_monitor.db")
    ...
    try:
        ...
        db = Database(db_path)
        db.connect()
        cost_data = db.get_daily_cost_data(days=30)
        db.close()

...
class Database:
    ...
    def _create_auxiliary_tables(self, cursor: sqlite3.Cursor) -> None:
        ...
        # Daily energy counters (resets at midnight)
        cursor.execute(
            """
        ...
        # Cumulative energy counters (historical, never resets)
        cursor.execute(
            """
        ...
        with self._lock:
            ...
            cursor.execute(
                ...
                INSERT OR REPLACE INTO daily_energy (
                    date, last_update, total_wh, gpu_wh, cpu_wh, session_cost_usd
                )
                VALUES (?, datetime('now'), ?, ?, ?, ?)
                """,
                (date, total_wh, gpu_wh, cpu_wh, session_cost_usd),
            )
            self.conn.commit()

    def get_daily_energy(self, date: str) -> Optional[Dict[str, Any]]:
        """Get energy values for a specific date.

        Args:
            date: Date string (YYYY-MM-DD)

        Returns:
            Dictionary of energy values or None if not found
        """
        cursor = self.conn.cursor()
        cursor.execute(
            """
            SELECT date, last_update, total_wh, gpu_wh, cpu_wh, session_cost_usd
            FROM daily_energy
            WHERE date = ?
            """
        , (date,))
    ...
    def get_today_energy(self) -> Optional[Dict[str, Any]]:
        """Get energy values for today.

        Returns:
            Dictionary of energy values or None if not found
        """
        today = time.strftime("%Y-%m-%d", time.localtime())
        return self.get_daily_energy(today)

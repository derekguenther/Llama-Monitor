...
class ElectricityCostCalculator:
    ...
    def __init__(
        ...
        # Session tracking (cumulative for entire session)
    ...
    def _load_today_energy(self) -> None:
        """Load today's energy from database and continue accumulating.

        If the date has changed (midnight passed), resets to today.
        Otherwise, loads stored values and continues accumulating.
        """
        today_date = self._get_today_date()

        # Check if we already loaded today's data and it's still today
        if self.today_date == today_date:
            return  # Already loaded today's data

        # Load today's energy from database
        today_data = self.database.get_today_energy()

        if today_data:
            # Today's data exists - continue accumulating from where we left off
            self.today_energy_wh = today_data.get("total_wh", 0.0)
            self.today_gpu_wh = today_data.get("gpu_wh", 0.0)
            self.today_cpu_wh = today_data.get("cpu_wh", 0.0)
            self.today_date = today_date
        else:
            # No today's data - start fresh for today
            self.today_energy_wh = 0.0
            self.today_gpu_wh = 0.0
            self.today_cpu_wh = 0.0
            self.today_date = today_date

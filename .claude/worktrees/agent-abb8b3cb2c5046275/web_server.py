...
def api_historical_metrics():
    """Return historical metrics for a predefined timeframe.
    ...
    Query params:
    - timeframe: One of the predefined timeframes (default: last_hour)
    ...
    time_ranges = {
        ...
        "last_2_hours": 7200,        # 2 hours
        "last_6_hours": 21600,       # 6 hours
        "last_day": 86400,           # 24 hours
        "last_3_days": 259200,       # 3 days
        "last_week": 604800,         # 7 days
        "last_2_weeks": 1209600,     # 14 days
        "last_month": 2592000,       # 30 days
        "last_3_months": 7776000,    # 90 days
        "last_year": 31536000,       # 365 days
    }

    if timeframe not in time_ranges:
        timeframe = "last_hour"

    seconds = time_ranges[timeframe]
    ...
    try:
        ...
        # Get all data within the time range, ordered by timestamp ASC for charts
        query = """
            SELECT timestamp, server_data, system_data, cost_data
            FROM combined_metrics
        ...
        for row in rows:
            ...
            # Extract GPU usage, CPU usage, and power values
            gpu_usage = system_data.get("gpu_usage", 0) or 0
            cpu_percent = system_data.get("cpu_percent", 0) or 0
            gpu_power = system_data.get("gpu_power_w", 0) or 0
            cpu_power = system_data.get("cpu_power_w", 0) or 0

            # Convert timestamp to readable format
            try:
                ts = int(row["timestamp"]) if isinstance(row["timestamp"], str) else row["timestamp"]

...
2) Review for completeness: If gaps → mark needs-human-input (`bd update <id> --status blocked --add-label needs-human-input --append-notes "NEEDS HUMAN INPUT: ..."`), consolidate questions in description, pick next Open bead
...
4) Work the bead
   ...
   c) Complications → mark needs-human-input (`--status blocked --add-label needs-human-input --append-notes "NEEDS HUMAN INPUT: <questions>"`), pick next Open bead
   d) Create appropriate tests and run them — all tests must pass before adding the `needs_review` label
   e) Success → add `needs_review` label (`bd update <id> --add-label needs_review`)
5) Repeat until no Open beads remain → print summary, start Review Process

## Review Process

1) Find a needs-review bead: `bd list --label needs_review`
...
4) Functional review — start main.py from within worktree, test with chrome superpower
   - **IF CHROME IS NOT AVAILABLE**: mark needs-human-input (`--status blocked --add-label needs-human-input`) with question "Chrome unavailable for functional review. Proceed with code review + tests only, or fix Chrome first?" Do NOT skip functional review silently.
...
1. `git checkout main`
2. `git merge --squash <bead-name>` — if this fails, do NOT close the bead; mark needs-human-input (`--status blocked --add-label needs-human-input`) with merge details and questions
3. `git commit -m "Close <bead-name>: <brief description>"`
4. Run tests to verify nothing broke — if they fail, mark needs-human-input (`--status blocked --add-label needs-human-input`)
...
- **NEVER silently skip a required process step** — if you can't complete a step (e.g., Chrome unavailable, tests failing), mark needs-human-input (`--status blocked --add-label needs-human-input`) and ask

## Chrome Debugging Setup

**Important**: The Chrome MCP server (superpowers-chrome) is designed to **auto-start Chrome** when needed. You do NOT need to start Chrome manually before running the MCP server.

### If Chrome Crashes or Leaves Stale Lock Files

If OpenCode crashes or Chrome fails to start with "Chrome did not become ready" error, clean up stale lock files:

```bash
# Remove stale lock files left by crashed Chrome instances
...
Then restart the MCP server - Chrome will start fresh.

### Manual Chrome Start (Only for Testing)

If you need to manually start Chrome for testing purposes:
...
**Note**: The Chrome MCP server will auto-start Chrome when needed. If you encounter "Chrome did not become ready" errors, clean up stale lock files first (see Chrome Debugging Setup above).
